        ...
        {user ? (
          <div className="flex items-center space-x-10">
            <Link href="/api/auth/logout" className='absolute left-5 rounded-full items-center bg-neutral-600 hover:bg-neutral-700 p-3 w-12 h-12 cursor-pointer'>
              <img src='logout_icon_48dp.png' />
            </Link>
            <img alt='PolizeiLogo' className='w-32' src='/SH_Polizei_Logo.png' />
            <img alt="profile" className="absolute right-5 rounded-full w-12 h-12" src={user.picture ? user.picture : ''} />
          </div>
        ) : (
